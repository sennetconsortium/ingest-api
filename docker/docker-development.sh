...
if [[ "$1" != "check" && "$1" != "config" && "$1" != "build" && "$1" != "start" && "$1" != "stop" && "$1" != "down" ]]; then
    ...
    if [ "$1" = "check" ]; then
        ...
        absent_or_newer ingest-api/src ../src

        echo 'Checks complete, all good :)'
    elif [ "$1" = "config" ]; then
        docker-compose -f docker-compose.yml -f docker-compose.development.yml -p ingest-api config
    elif [ "$1" = "build" ]; then
        # Delete the copied source code dir if exists
        if [ -d "ingest-api/src" ]; then
            rm -rf ingest-api/src
        fi
