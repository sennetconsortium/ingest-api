...
def copy_protect_files_to_public(job_id: str, dataset: dict, ancestor_path: str) -> JobResult:
    ...
    if dataset.get("creation_action") == "Multi-Assay Split" and "protected" not in ancestor_path:
        return JobResult(
            success=True,
            results="Dataset is a component dataset with a public primary dataset, a symlink was created instead",
        )

    # If the dataset is not a component and is not protected
    if (
        dataset.get("creation_action") != "Multi-Assay Split"
        and dataset.get("data_access_level") != "protected"
    ):
        return JobResult(
            success=True,
            results="Dataset is not a component dataset or protected, no files to copy",
        )
