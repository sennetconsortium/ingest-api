...
from jobs.cache.datasets import (
    DATASETS_DATASTATUS_JOB_PREFIX,
...
def create_dataset(suppress_reindex: bool):
    ...
    try:
        ...
        requested_group_uuid = auth_helper_instance.get_write_group_uuid(
            token, requested_group_uuid
        )
        ...
        ingest_helper.create_dataset_directory(
            new_dataset, requested_group_uuid, new_dataset["uuid"]
        )
        ...
        return Response(
            "Unexpected error while creating a dataset: " + str(e) + "  Check the logs", 500
        )
    ...
    try:
        ...
        for dataset in json_data_dict.get("datasets"):
            if "dataset_link_abs_dir" in dataset:
                ...
                return Response(
                    "Required field 'dataset_link_abs_dir' is missing from dataset", 400
                )

            if "contains_human_genetic_sequences" not in dataset:
                return Response(
                    "Missing required keys in request json: datasets.contains_human_genetic_sequences",
                    400,
                )
        ...
        for dataset in new_datasets_list:
            ...
            if "dataset_link_abs_dir" in dataset:
                ...
                path = normalize_globus_path(dataset["dataset_link_abs_dir"])
                new_directory_path = ingest_helper.get_dataset_directory_absolute_path(
                    dataset, requested_group_uuid, dataset["uuid"]
                )
                logger.info(
                    f"Creating a directory as: {new_directory_path} with a symbolic link to: {path}"
                )
...
def bulk_update_datasets_uploads(entities: list, token: str, user: User):
    ...
    diff = uuids.difference(
        {e["uuid"] for e in db_entities if equals(e["entity_type"], entity_type)}
    )
    if len(diff) > 0:
...
def submit_datasets_from_bulk(uuids: list, token: str, user: User, suppress_reindex: bool):
    ...
    diff = set(uuids).difference({ds["uuid"] for ds in datasets})
    if len(diff) > 0:
        abort_not_found(f"No datasets found with the following uuids: {', '.join(diff)}")

    bad_uuids = [ds["uuid"] for ds in datasets if ds["status"] in ["Published", "Processing"]]
    if bad_uuids:
...
def get_file_system_absolute_path(uuid: str):
    ...
    if equals(entity_type, Ontology.ops().entities().UPLOAD):
        path = ingest_helper.get_upload_directory_absolute_path(
            group_uuid=group_uuid, upload_uuid=uuid
        )
    ...
    try:
        ...
        return Response(
            f"Error while getting file-system-abs-path for entity with uuid {uuid} "
            + hte.get_description(),
            hte.get_status_code(),
        )
...
def get_multiple_file_system_absolute_paths(uuids: list):
    ...
    try:
        ...
        entity_types = ["Dataset", "Upload"]
        entities = Neo4jHelper.run_query(
            q, uuids=uuids, entity_types=entity_types, limit=len(uuids)
        )
    except Exception as e:
        logger.error(f"Error while retrieving entities' file system absolute paths: {str(e)}")
        abort_internal_err("Error while retrieving entities' file system absolute paths")
    ...
    for ds_uuid in ds_uuid_list:
        try:
            ...
            if ent_type_m is None or ent_type_m.strip() == "":
                error_id = {
                    "id": ds_uuid,
                    "message": "id not for Dataset, Publication or Upload",
                    "status_code": 400,
                }
            ...
            ingest_helper = IngestFileHelper(current_app.config)
            if ent_type == "upload":
                path = ingest_helper.get_upload_directory_relative_path(
                ...
                if group_uuid is None:
                    error_id = {
                        "id": ds_uuid,
                        "message": "Unable to find group uuid on dataset",
                        "status_code": 400,
                    }
                    error_id_list.append(error_id)
                if is_phi is None:
...
def get_ds_path(ds_uuid: str, ingest_helper: IngestFileHelper) -> str:
    ...
    if ent_type.lower().strip() == "upload":
        return ingest_helper.get_upload_directory_absolute_path(
            group_uuid=group_uuid, upload_uuid=ds_uuid
        )
    is_phi = __get_dict_prop(dset, "contains_human_genetic_sequences")
...
def submit_dataset(uuid):
    ...
    try:
        ...
        if "hmgroupids" not in user_info:
            return Response(
                "user not authorized to submit data, unable to retrieve any group information", 403
            )
    ...
    def call_entity_api():
        return requests.put(
            entity_api_url, json=dataset_request, headers=get_auth_header_dict(token), verify=False
        )
    ...
    def change_status_and_call_entity_api(message=None, status="Error"):
        try:
            ...
            logger.error(hte2)
            return Response(
                f"HTTPException while updating Dataset: {str(hte2)}. Check the logs.", 500
            )
    ...
    def call_airflow():
        ...
        try:
            ...
            if r.ok:
                ...
                logger.info(
                    "Time to get response from airflow: " + str(airflow_second_stop - airflow_start)
                )
    ...
    try:
        ...
        with Neo4jHelper.get_instance().session() as neo_session:
                ...
                if entity_type == "Sample":
                    ...
                    if is_primary:
                        if metadata is None or metadata.strip() == "":
                            return (
                                jsonify(
                                    {"error": f"source.metadata is missing for {dataset_uuid}"}
                                ),
                                400,
                            )
            ...
            if (
                ...
                abort_bad_req(
                    f"{dataset_uuid}: dataset of organ {organ} must have a rui_location associated with it or have an exemption. Will not Publish"
                )
            ...
            if dataset_data_access_level == "consortium" and not is_component:
            ...
            if is_primary or has_entity_lab_processed_dataset_type:
                ...
                try:
                    datacite_doi_helper.create_dataset_draft_doi(
                        entity_dict, check_publication_status=False
                    )
                except Exception as e:
                    logger.exception(
                        f"Exception while creating a draft doi for {dataset_uuid}: {e}"
                    )
                    return (
                        jsonify(
                            {
                                "error": f"Error occurred while trying to create a draft doi for {dataset_uuid}. {e}"
                            }
                        ),
                        500,
                    )
                ...
                try:
                    ...
                    return (
                        jsonify(
                            {
                                "error": f"Error occurred while trying to change doi draft state to findable doi for {dataset_uuid}. {e}"
                            }
                        ),
                        500,
                    )
            ...
            logger.info(
                dataset_uuid + "\t" + dataset_uuid + "\tNEO4J-update-base-dataset\t" + update_q
            )
            ...
            if len(uuids_for_public) > 0:
                update_q = (
                    "MATCH (e:Entity) WHERE e.uuid IN $uuids SET e.data_access_level = 'public'"
                )
                logger.info(
                    identifier + "\t" + dataset_uuid + "\tNEO4J-update-ancestors\t" + update_q
                )
        ...
        if is_primary or is_component is False:
            ...
            logger.info(
                f"publish_datastage; writing metadata.json file: '{md_file}'; containing: '{json_object}'"
            )
            try:
                ...
                return (
                    jsonify({"error": f"{dataset_uuid} problem writing metadata.json file."}),
                    500,
                )

        # Change the directory permissions to prevent user from writing to published folder
        acls_cmd = ingest_helper.set_dataset_permissions(
            dataset_uuid, dataset_group_uuid, data_access_level, True, no_indexing_and_acls
        )
        ...
        if not no_indexing_and_acls:
            for source_uuid in sources_to_reindex:
                try:
                    rspn = requests.put(
                        current_app.config["SEARCH_WEBSERVICE_URL"] + "/reindex/" + source_uuid,
...
def create_uploadstage():
    ...
    try:
        ...
        logger.error(e, exc_info=True)
        return Response(
            "Unexpected error while creating a upload: " + str(e) + "  Check the logs", 500
        )
