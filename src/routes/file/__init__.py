...
def get_upload_file_helper_instance() -> UploadFileHelper:
    try:
        if UploadFileHelper.is_initialized() is False:
            ...
            return UploadFileHelper.create(
...
def upload_file():
    ...
    try:
        ...
        rspn_data = {"temp_file_id": temp_id}
...
def commit_file():
    ...
    source_file_path = os.path.join(
        str(current_app.config["FILE_UPLOAD_DIR"]), entity_uuid, file_uuid, filename
    )
    ...
    try:
        ...
        logger.exception(
            f"Failed to create the symbolic link from {source_file_path} to {target_file_path}"
        )
...
def remove_file():
    ...
    for file_uuid in file_uuids:
        ...
        files_info_list = file_upload_helper_instance.remove_file(
            entity_upload_dir, file_uuid, files_info_list
        )

        # Also remove the dir contains the symlink to the uploaded file under assets
        # /hive/hubmap/assets/<file_uuid> (for PROD)
        assets_file_dir = os.path.join(
            str(current_app.config["SENNET_WEBSERVICE_FILEPATH"]), file_uuid
        )
