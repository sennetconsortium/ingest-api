...
def get_globus_url(data_access_level, group_name, uuid):
    ...
    if data_access_level == "public":
        ...
        access_dir = commons_file_helper.ensureTrailingSlashURL(
            current_app.config["RELATIVE_GLOBUS_PUBLIC_ENDPOINT_FILEPATH"]
        )
    ...
    # protected access
        ...
        access_dir = commons_file_helper.ensureTrailingSlashURL(
            current_app.config["RELATIVE_GLOBUS_PROTECTED_ENDPOINT_FILEPATH"]
        )
        dir_path = dir_path + access_dir + group_name + "/"
    ...
    if globus_server_uuid is not None:
        ...
        globus_url = (
            commons_file_helper.ensureTrailingSlash(current_app.config["GLOBUS_APP_BASE_URL"])
