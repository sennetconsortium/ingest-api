...
class DatasetHelper:
    def __init__(self, config):
        ...
        self.auth_helper_instance = AuthHelper.configured_instance(
        ...
        try:
            self.neo4j_driver_instance = neo4j_driver.instance(
                self.appconfig["NEO4J_SERVER"],
        ...
        with self.neo4j_driver_instance.session() as session:
            ...
            for record in records:
                ...
                if group_uuid is None:
                    return Response(
                        f"Unable to process submit.  group_uuid not found on entity:{uuid}", 400
                    )
            if count == 0:
                return Response(f"Dataset with uuid:{uuid} not found.", 404)
            return group_uuid

    def get_datasets_by_uuid(
        self, uuids: Union[str, Iterable], fields: Union[dict, Iterable, None] = None
    ) -> Optional[list]:
    ...
    def update_ingest_status_title_thumbnail(
        ...
        try:
            ...
            try:
                ...
                updated_ds["thumbnail_file_to_add"] = {"temp_file_id": temp_file_id}
            ...
            self.logger.info(
                f"No existing thumbnail file found for the dataset uuid {dataset_uuid}"
            )
            pass
    ...
    def handle_thumbnail_file(
        ...
        try:
            ...
            self.logger.info(
                f"Successfully removed the existing thumbnail file of the dataset uuid {dataset_uuid}"
            )
        except KeyError:
            self.logger.info(
                f"No existing thumbnail file found for the dataset uuid {dataset_uuid}"
    ...
    def get_dataset_ingest_update_record(self, json_data):
        ...
        if "overwrite_metadata" in json_data and json_data["overwrite_metadata"] == False:
            raise ValueError(
                "overwrite_metadata set to False, merging of metadata is not supported on update"
            )
    ...
    def create_ingest_payload(self, dataset):
        ...
        full_path = self.ingest_helper.get_dataset_directory_absolute_path(
            dataset, dataset["group_uuid"], dataset["uuid"]
        )
        return {
