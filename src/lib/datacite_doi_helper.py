...
class DataCiteDoiHelper:
    ...
    def check_doi_existence_and_state(self, entity: dict):
        datacite_api = DataCiteApi(
            self.datacite_repository_id,
            self.datacite_repository_password,
            self.datacite_sennet_prefix,
            self.datacite_api_url,
            self.entity_api_url,
        )
        ...
        if ("entity_type" in dataset) and (dataset["entity_type"] == "Dataset"):
            ...
            if "published_timestamp" in dataset:
                ...
                publication_year = int(
                    datetime.fromtimestamp(dataset["published_timestamp"] / 1000).year
                )
            ...
            if response.status_code == 201:
                logger.info(
                    f"======Created draft DOI for dataset {dataset['uuid']} via DataCite======"
                )
                ...
                logger.exception(
                    f"Unable to create draft DOI for dataset {dataset['uuid']} via DataCite"
                )
            ...
            raise KeyError(
                "Either the entity_type of the given Dataset is missing or the entity is not a Dataset"
            )
    ...
    def create_collection_draft_doi(self, collection: dict) -> object:
        ...
        if response.status_code == 201:
            logger.info(
                f"======Created draft DOI for collection {collection['uuid']} via DataCite======"
            )
            doi_data = response.json()
            logger.debug("======resulting json from DataCite======")
            logger.debug(doi_data)
            ...
            # Log the full stack trace, prepend a line with our message
            logger.exception(
                f"Unable to create draft DOI for collection {collection['uuid']} via DataCite"
            )
            logger.debug(f"======Status code from DataCite {response.status_code} ======")
            logger.debug("======response text from DataCite======")
            logger.debug(response.text)
    ...
    def is_invalid_doi(self, protocol):
        ...
        selection_protocol_pattern2 = re.match(
            "^dx\.doi\.org/[\d]+\.[\d]+/protocols\.io\..*$", protocol
        )
